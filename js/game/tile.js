...
define(['jquery', 'game/util'], function ($, util) {
    ...
    Tile.prototype.draw = function () {
        ...
        this.drawElevationCircle(ctx, this.bounds[0]);
        this.drawElevationCircle(ctx, this.bounds[1]);
        this.drawElevationCircle(ctx, [this.bounds[0][0], this.bounds[1][1]]);
        this.drawElevationCircle(ctx, [this.bounds[1][0], this.bounds[0][1]]);

        ctx.restore();
    };

    Tile.prototype.drawElevationCircle = function (ctx, position) {
        ...
        ctx.beginPath();
        ctx.arc(canvasPosition[0], canvasPosition[1], this.getCanvasWidth() / 2, 0, Math.TAU, true);
        ctx.closePath();
        ctx.fill();
    };
